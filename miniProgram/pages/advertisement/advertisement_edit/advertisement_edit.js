// pages/advertisement/advertisement_edit/advertisement_edit.js
wx.cloud.init({
  env: 'cloudpg-v0ty3'
})
...
Page({
  ...
  uploadding: function (e) {
    ...
    var files = this.data.update//files是要上传的图片
    if (e == files.length) {
      db.collection('advertisement').doc(this.data.list._id).update({
        data: {
  ...
  ,
  deletee: function(e){
    db.collection("advertisement").doc(this.data.list._id).remove({
      success: res=>{
        wx.showToast({
          title: '删除成功！',
        })
